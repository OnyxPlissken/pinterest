...
export async function getAdminState() {
  ...
  const rules = rulesStore.store.rules.map(sanitizeRule);
...
export async function updateRule(ruleId, input) {
  ...
  if (!nextRule.active && settingsStore.settings.defaultRuleId === nextRule.id) {
    const fallbackRule = nextRules.find((rule) => rule.active);
    await saveSettingsStore(
      {
        ...settingsStore.settings,
        defaultRuleId: fallbackRule?.id || ""
      },
