...
import { setPinterestTokenRefresher } from "./pinterest-api";
import {
...
const DEFAULT_APP_NAME = "ISAIA Pin Manager";
const RULE_SELECTION_MODES = new Set(["filenameLook", "imageSequence"]);
const DEFAULT_SELECTION_MODE = "filenameLook";
const IMAGE_SEQUENCE_RULE_ID = "regola-ogni-immagine-look";
const LEGACY_APP_NAMES = new Set([
  "Pinterest Assets Management",
  "ISAIA Pinterest Assets Management",
  "ISAIA SharePoint Pinterest Manager",
  "ISAIA SharePoint Pin Manager"
]);

async function refreshPinterestAccessToken(config) {
  const appId = config?.pinterest?.appId || "";
  const appSecret = config?.pinterest?.appSecret || "";
  const refreshToken = config?.pinterest?.refreshToken || "";
...
setPinterestTokenRefresher(refreshPinterestAccessToken);
...
function normalizeAppName(value) {
  ...
  return !normalized || LEGACY_APP_NAMES.has(normalized) ? DEFAULT_APP_NAME : normalized;
}
...
function normalizeSelectionMode(value, fallback = DEFAULT_SELECTION_MODE) {
  return RULE_SELECTION_MODES.has(value) ? value : fallback;
}
...
function buildImageSequenceRule(config) {
  const now = new Date().toISOString();

  return {
    id: IMAGE_SEQUENCE_RULE_ID,
    name: "Ogni immagine = Look",
    titlePrefix: config.pinterest.titlePrefix,
    descriptionPrefix: config.pinterest.descriptionPrefix,
    linkUrl: config.pinterest.linkUrl,
    thumbnailMode: config.pinterest.thumbnailMode,
    selectionMode: "imageSequence",
    active: true,
    createdAt: now,
    updatedAt: now,
    usageDescription: "Conta tutte le immagini della cartella finale e le numera come Look 1, Look 2, Look 3..."
  };
}
...
function preferEnvironmentCredential(envValue, storedValue) {
  return normalizeText(envValue) || normalizeText(storedValue);
}
...
function sanitizeRule(rule, defaultRuleId = "") {
  ...
  return {
    ...
    selectionModeLabel:
      rule.selectionMode === "imageSequence"
        ? "Ogni immagine diventa un Look progressivo"
        : "LOOK letto dal nome file",
...
function sanitizeSettings(settings) {
  ...
  const effectivePinterestAppId = preferEnvironmentCredential(
    config.pinterest.appId,
    settings.pinterest.appId
  );
  const effectivePinterestAppSecret = preferEnvironmentCredential(
    config.pinterest.appSecret,
    settings.pinterest.appSecret
  );
  const effectivePinterestAccessToken =
    normalizeText(settings.pinterest.accessToken) || normalizeText(config.pinterest.accessToken);
  const effectivePinterestRefreshToken =
    normalizeText(settings.pinterest.refreshToken) || normalizeText(config.pinterest.refreshToken);
  ...
  return {
    ...
    pinterest: {
      appId: effectivePinterestAppId,
      appSecretConfigured: Boolean(effectivePinterestAppSecret),
      accessTokenConfigured: Boolean(effectivePinterestAccessToken),
      refreshTokenConfigured: Boolean(effectivePinterestRefreshToken),
...
function buildEmptyRulesStore(config) {
  return {
    version: 1,
    rules: [buildDefaultRule(config), buildImageSequenceRule(config)]
  };
}
...
function normalizeRule(rule, fallbackRule) {
  return {
    ...
    selectionMode: normalizeSelectionMode(rule?.selectionMode, fallbackRule.selectionMode),
...
function ensureBuiltInRules(rules, config) {
  const ruleIds = new Set(rules.map((rule) => rule.id));

  if (ruleIds.has(IMAGE_SEQUENCE_RULE_ID)) {
    return rules;
  }

  return [
    ...rules,
    normalizeRule(buildImageSequenceRule(config), buildDefaultRule(config))
  ];
}
...
async function loadRulesStore() {
  ...
  const normalizedRules = ensureBuiltInRules(incomingRules.length
    ? incomingRules.map((rule) => normalizeRule(rule, defaultRule))
    : fallbackStore.rules.map((rule) => normalizeRule(rule, defaultRule)), config);
  const nextStore = {
    version: 1,
...
async function loadSettingsStore(rulesStore) {
  ...
  const nextSettings = {
    version: 1,
    appName: normalizeAppName(payload.document.appName),
    ...
    pinterest: {
      ...
      refreshToken:
        normalizeText(payload.document.pinterest?.refreshToken) ||
        defaultSettings.pinterest.refreshToken,
      titlePrefix:
      ...
      linkUrl: hasOwn(payload.document.pinterest, "linkUrl")
        ? normalizeOptionalUrl(payload.document.pinterest.linkUrl)
...
export async function updateRule(ruleId, input) {
  ...
  const nextRule = {
    ...
    selectionMode: normalizeSelectionMode(input.selectionMode, target.selectionMode),
    active: input.active !== false,
    usageDescription: normalizeText(input.usageDescription ?? input.notes),
...
export async function updateOperationalSettings(input) {
  ...
  const nextSettings = {
    ...settingsStore.settings,
    appName: normalizeAppName(input.appName) || settingsStore.settings.appName,
    defaultRuleId: nextDefaultRuleId,
    sharePoint: {
      driveName:
    ...
    pinterest: {
      ...
      linkUrl:
        hasOwn(pinterestInput, "linkUrl")
          ? normalizeOptionalUrl(pinterestInput.linkUrl)
...
export async function updatePinterestTokens({ accessToken, refreshToken = "" }) {
  const rulesStore = await loadRulesStore();
  const settingsStore = await loadSettingsStore(rulesStore);
  ...
  const nextSettings = {
    ...
    pinterest: {
      ...
      refreshToken: normalizedRefreshToken || settingsStore.settings.pinterest.refreshToken || ""
    }
  };
...
export async function getRuntimeConfig(ruleId = "") {
  ...
  return {
    config: {
      ...
      pinterest: {
        ...config.pinterest,
        appId: preferEnvironmentCredential(config.pinterest.appId, privateSettings.pinterest.appId),
        appSecret: preferEnvironmentCredential(
          config.pinterest.appSecret,
          privateSettings.pinterest.appSecret
        ),
        accessToken: privateSettings.pinterest.accessToken || config.pinterest.accessToken,
        refreshToken: privateSettings.pinterest.refreshToken || config.pinterest.refreshToken,
