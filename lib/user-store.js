...
async function saveStore(store, persistent) {
  if (!persistent) {
    throw new Error("Storage utenti non configurato in modo persistente.");
  }
...
export async function touchUserLastLogin(userId) {
  const { store, persistent } = await loadStore();
  const now = new Date().toISOString();
  ...
  await saveStore({ ...store, users: nextUsers }, persistent);
}
