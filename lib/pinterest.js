...
function buildRowData(item, mediaUrl, config) {
  ...
  const sectionName = assetTypeLabel;

  return {
    Title: `${config.pinterest.titlePrefix} ${item.season} ${assetTypeLabel} | ${lookLabel}`.trim(),
    "Media URL": mediaUrl,
...
export async function loadPinterestSelection({ subPaths = [], subPath = "", origin = "", ruleId = "" } = {}) {
