...
async function getAccessTokenForScope(scope) {
  ...
  const tokenEndpoint = `https://login.microsoftonline.com/${sharePoint.tenantId}/oauth2/v2.0/token`;
  let body;
  ...
  if (!response.ok || !payload.access_token) {
    throw new Error(
      `Autenticazione Microsoft fallita (${response.status}): ${payload.error_description ?? payload.error ?? "errore sconosciuto"}`
    );
  }
...
export async function getSharePointAccessToken() {
  const { sharePoint } = getConfig();
  return getAccessTokenForScope(`https://${sharePoint.hostname}/.default`);
}
