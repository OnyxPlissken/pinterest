...
function buildStoredPendingPin(target, batchId, matchedPin = null) {
  return {
    ...
    csvBatchId: batchId,
    status: "pending_csv_upload",
    syncedAt: new Date().toISOString()
  };
}

function buildStoredImportedPin(target, pin, board, section, batchId) {
  return {
    sourceKey: target.sourceKey,
    sourceSubPath: target.sourceSubPath,
    serverRelativeUrl: target.serverRelativeUrl,
...
function classifyTarget(target, stored, pinterestMatch, strategy) {
  const existing = stored || pinterestMatch;
  ...
  return {
    ...
    target,
    pinId: existing.pinId || "",
    reason: existing.status === "pending_csv_upload" ? "Gia esportato in CSV e in attesa upload." : "Gia tracciato e invariato."
  };
}
...
async function buildCsvAssistPlan({ subPaths = [], subPath = "", origin = "", ruleId = "", strategy = "newOnly" } = {}) {
  ...
  const pinterestIndex = await loadPinterestPinIndex(selection.config);
  ...
  for (const target of targets) {
    ...
    const pinterestMatch = pinterestIndex.get(target.matchKey) || null;
...
export async function generateCsvAssist(options = {}) {
  const plan = await buildCsvAssistPlan(options);
  const directImport = options.directImport === true;
  ...
  for (const action of plan.actions) {
    try {
      ...
      if (directImport) {
        const created = await createPinForTarget(plan.selection.config, cache, action.target);
        nextPins[action.target.sourceKey] = buildStoredImportedPin(
          action.target,
          created.pin,
          created.board,
          created.section,
  ...
  return {
    ...
    selectedSubPaths: plan.selection.selectedSubPaths,
    scannedCount: plan.selection.files.length,
    generatedCount: directImport ? results.filter((result) => result.imported).length : rows.length,
    skippedCount: plan.selection.skipped.length,
    summary: summarizeActions(results),
    actions: results.slice(0, 200),
    csvContent: !directImport && rows.length ? serializeCsv(rows) : "",
    csvFilename: !directImport && rows.length ? `pinterest-csv-assist-${timestamp}.csv` : "",
    csvBatchId: batchId,
