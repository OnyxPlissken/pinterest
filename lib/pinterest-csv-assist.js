...
function classifyTarget(target, stored, pinterestMatch, strategy) {
  ...
  if (strategy === "regenerateSelection") {
    return {
      type: existing.pinId ? "replace" : "create",
      target,
      pinId: existing.pinId || "",
      matchedPin: existing.pinId ? existing : null,
      reason: existing.pinId
        ? "Rigenerazione richiesta per la selezione."
        : "Rigenerazione richiesta: il vecchio Pin non e identificato, quindi genero la riga CSV senza delete."
    };
  }
      ...
      reason: imageChanged
        ...
        : metadataChanged
          ...
          : existing.pinId
            ...
            : `File rinominato o spostato. ${missingPinReason}`
    };
  }
