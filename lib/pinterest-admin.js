...
export async function movePinterestAdminPins(config, { pinIds, boardId, sectionId = "" }) {
  const ids = Array.from(new Set((pinIds || []).map(String).filter(Boolean)));

  if (!ids.length) {
    throw new Error("Seleziona almeno un Pin da spostare.");
  }

  if (!boardId) {
    throw new Error("Seleziona una bacheca di destinazione.");
  }
  ...
  for (const pinId of ids) {
    try {
      ...
      results.push({
        pinId,
        status: "error",
        error: error instanceof Error ? error.message : "Errore spostamento Pin."
      });
    }
  }
...
export function summarizePinterestAdminResults(results) {
  return {
