...
const PIN_EDIT_RESTRICTED_MESSAGE =
  "Pinterest ha bloccato la modifica dei Pin: l'app non ha accesso alla feature ristretta pin_edit. Finche Pinterest non approva questa capability, titolo, descrizione, link, privacy e spostamenti Pin via API non sono disponibili. Puoi eliminare i Pin e rigenerarli via CSV, oppure richiedere a Pinterest l'accesso a questa feature.";

function isPinEditRestrictedError(error) {
  const message = error instanceof Error ? error.message : String(error || "");
  const normalized = message.toLowerCase();

  return (
    normalized.includes("pin_edit") &&
    (normalized.includes("restricted feature") || normalized.includes("feature ristretta"))
  );
}

function normalizePinterestAdminError(error) {
  if (isPinEditRestrictedError(error)) {
    return new Error(PIN_EDIT_RESTRICTED_MESSAGE);
  }
...
export async function updatePinterestAdminPin(
  ...
  try {
    ...
    throw normalizePinterestAdminError(error);
  }
...
export async function bulkUpdatePinterestAdminPins(config, { updates }) {
  ...
  const pinEditRestricted = results.some((result) => isPinEditRestrictedError(result.error));

  return {
    ok: results.filter((result) => result.status === "ok").length,
    failed: results.filter((result) => result.status === "error").length,
    restrictedFeature: pinEditRestricted ? "pin_edit" : "",
    message: pinEditRestricted ? PIN_EDIT_RESTRICTED_MESSAGE : "",
    results
  };
}
