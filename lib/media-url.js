...
function isTiffFilename(filename) {
  return [".tif", ".tiff"].includes(path.posix.extname(filename).toLowerCase());
}
...
export function resolvePublicOrigin(origin = "") {
  ...
  const configuredOrigin = normalizeOrigin(config.app.publicOrigin);
  const fallbackOrigin =
    process.env.VERCEL_PROJECT_PRODUCTION_URL ||
    process.env.VERCEL_URL ||
    "http://localhost:3000";

  return configuredOrigin || normalizeOrigin(origin) || normalizeOrigin(fallbackOrigin);
}
...
export function buildPublicMediaUrl(serverRelativeUrl, origin = "", versionHint = "") {
  ...
  const publicFilename = isTiffFilename(filename)
    ? `${path.posix.basename(filename, path.posix.extname(filename))}.jpg`
    : filename;
  const url = new URL(`/media/${encodeURIComponent(publicFilename)}`, resolvePublicOrigin(origin));
