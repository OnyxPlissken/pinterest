...
function normalizeReplacementMode(value) {
  return String(value || "").toLowerCase() === "skip" ? "skip" : "replace";
}
...
function mergeExistingMatch(stored, pinterestMatch) {
  ...
  return {
    ...
    mediaUrl: stored.mediaUrl || pinterestMatch.mediaUrl || "",
  ...
  for (const board of targetBoards) {
    ...
    for (const pin of boardPins) {
      ...
      const entry = {
        ...
        pinId: pin.id,
        pinUrl: pin.url || "",
        mediaUrl: getPinterestPinImageUrl(pin),
        boardId: board.id,
        boardName: board.name,
...
function getPinterestPinImageUrl(pin) {
...
function buildReplacementAction(target, existing, reason) {
  const discrepancies = Array.isArray(reason) ? reason : [reason].filter(Boolean);
  const reasonText = discrepancies.length
    ? discrepancies.join(", ")
    : "Dati modificati.";
  ...
  return {
    ...
    replacement: {
      ...
      currentPin: {
        ...
        link: existing.link || "",
        boardName: existing.boardName || target.metadata.boardName,
        sectionName: existing.sectionName || target.metadata.sectionName,
        mediaUrl: existing.mediaUrl || existing.imageUrl || "",
        privacy: existing.privacy || existing.boardPrivacy || ""
      },
      newPin: {
        title: target.metadata.title,
...
function buildSkippedReplacementAction(target, existing, reason) {
  return {
    ...buildReplacementAction(target, existing, reason),
    type: "skipped",
    message: "Sostituzione saltata su richiesta."
  };
}

function getMetadataDiscrepancies(target, existing) {
  const discrepancies = [];

  if (normalizeComparableText(existing?.title) !== normalizeComparableText(target.metadata.title)) {
    discrepancies.push("titolo");
  }

  if (
    normalizeComparableText(existing?.description) !==
    normalizeComparableText(target.metadata.description)
  ) {
    discrepancies.push("descrizione");
  }

  if (normalizeComparableUrl(existing?.link) !== normalizeComparableUrl(target.metadata.link)) {
    discrepancies.push("link");
  }

  if (normalizePinterestName(existing?.boardName) !== normalizePinterestName(target.metadata.boardName)) {
    discrepancies.push("bacheca");
  }

  if (
    normalizePinterestName(existing?.sectionName) !==
    normalizePinterestName(target.metadata.sectionName)
  ) {
    discrepancies.push("sezione");
  }
...
export async function syncPinterestPins({
  ...
  const selectedReplacementMode = normalizeReplacementMode(replacementMode);
  ...
  for (const target of targets) {
    ...
    try {
      ...
      const pendingStoredNeedsPinterestChange =
        ...
        (storedImageChanged || getMetadataDiscrepancies(target, stored).length > 0);
      ...
      if (liveMatch && (!stored || (!hasStoredPinterestPin(stored) && !pendingStoredNeedsPinterestChange))) {
        ...
        const liveDiscrepancies = getMetadataDiscrepancies(target, liveMatch);

        if (liveDiscrepancies.length) {
          const replacementReason = liveDiscrepancies.map((item) => `${item} modificato`);
          if (dryRun) {
            actions.push(buildReplacementAction(target, liveMatch, replacementReason));
            continue;
          }

          if (selectedReplacementMode === "skip") {
            actions.push(buildSkippedReplacementAction(target, liveMatch, replacementReason));
            continue;
          }

          await deletePinIfPresent(selection.config, liveMatch.pinId);
          const replaceTarget = preserveExistingPrivacy(target, liveMatch);
      ...
      const metadataDiscrepancies = getMetadataDiscrepancies(target, existing);
      const metadataChanged = metadataDiscrepancies.length > 0;
      ...
      if (imageChanged) {
        const replacementReason = [
          "media modificato",
          ...metadataDiscrepancies.map((item) => `${item} modificato`)
        ];
        if (dryRun) {
          actions.push(buildReplacementAction(target, existing, replacementReason));
          continue;
        }

        if (selectedReplacementMode === "skip") {
          actions.push(buildSkippedReplacementAction(target, existing, replacementReason));
          continue;
        }
      ...
      if (metadataChanged) {
        const replacementReason = metadataDiscrepancies.map((item) => `${item} modificato`);
        if (dryRun) {
          actions.push(buildReplacementAction(target, existing, replacementReason));
          continue;
        }
  ...
  const replacementCandidates = actions
