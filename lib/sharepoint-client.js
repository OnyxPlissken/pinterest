...
export function isTiffImage(filename) {
...
export async function collectImageFiles(
  ...
  const nestedFolders = (payload.Folders ?? []).filter(
    (folder) => folder.Exists && folder.Name !== "Forms"
  );
  const nestedFileGroups = await mapWithConcurrency(
    nestedFolders,
    4,
    (folder) =>
      collectImageFiles(token, config, folder.ServerRelativeUrl, [
        ...relativeSegments,
        folder.Name
      ])
  );
  files.push(...nestedFileGroups.flat());
